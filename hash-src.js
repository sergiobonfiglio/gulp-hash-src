...
var through = require("through2"),
    ...
    p,
    find_regex = /(href|src|ng-include|templateUrl|template)\s*([=:])\s*(?:("')([^']*)|(")([^"]*)|(')([^']*)|([^'"\s>]+))|url\s*\((?:(")([^"]+)|(')([^']+)|([^'")]+))/ig;
...
{
    var sep= match[2] || "=",
        quote = match[3] || match[5] || match[7] || "",
        link  = match[4] || match[6] || match[8] || match[9];
    /// Is this a href/src match?
    if (link) {
        return {
            prefix: match[1] + sep + quote,
            link: link,
            suffix: "", /// The last quote doesn't get matched
            abs: match[1] === 'templateUrl' ? true : false
        };
    }
    ...
    return {
        prefix: "url(" + match[10],
        link: match[11] || match[13] || match[14],
        suffix: "", /// The last quote doesn't get matched
    };
}
...
module.exports = function hash_src(options)
{
    ...
    if (!options.exts) {
        options.exts = [".html", ".js", ".css", ".jpg", ".jpeg", ".png", ".gif", ".webp", ".svg", ".pdf", ".ico", ".ttf", ".woff", ".mp3", ".ogg", ".ogv", ".mp4", ".webm", ".zip", ".tar", ".gz", ".bz2"];
    }
    if (!options.regex) {
    ...
    if (!options.fallback_hash) {
        options.fallback_hash = null;
    }
        ...
        girdle.async_loop(matches, cb, function oneach(match, next)
        {
            ...
            fs_helper.fs.exists(full_path, function onres(exists)
            {
                if (!exists) {
                    ...
                    hashes[link] = options.fallback_hash;
                    return next();
                }
